...
public class OverlaySwipeBack extends DraggableSwipeBack {
	...
	protected boolean onDownAllowDrag(int x, int y) {
		...
		case RIGHT:
			...
			return (!mSwipeBackViewVisible && initialMotionX >= width
					- mTouchSize)
					|| (mSwipeBackViewVisible && initialMotionX >= width
							+ mOffsetPixels);
		...
		case BOTTOM:
			final int height = getHeight();
			return (!mSwipeBackViewVisible && mInitialMotionY >= height
					- mTouchSize)
					|| (mSwipeBackViewVisible && mInitialMotionY >= height
							+ mOffsetPixels);
		default:
			break;
		}
	...
	protected boolean onMoveAllowDrag(int x, int y, float dx, float dy) {
		...
		switch (getPosition()) {
		case LEFT:
			return (!mSwipeBackViewVisible && mInitialMotionX <= mTouchSize && (dx > 0)) // Drawer
																							// closed
					|| (mSwipeBackViewVisible && x <= mOffsetPixels) // Drawer
																		// open
					|| (Math.abs(mOffsetPixels) <= mPeekSize && mSwipeBackViewVisible); // Drawer
																						// revealed

		case RIGHT:
			final int width = getWidth();
			return (!mSwipeBackViewVisible
					&& mInitialMotionX >= width - mTouchSize && (dx < 0))
		...
		case BOTTOM:
			final int height = getHeight();
			return (!mSwipeBackViewVisible
					&& mInitialMotionY >= height - mTouchSize && (dy < 0))
	...
	protected void onMoveEvent(float dx, float dy) {
		switch (getPosition()) {
		case LEFT:
			setOffsetPixels(Math.min(Math.max(mOffsetPixels + dx, 0),
					mSwipeBackViewSize));
			break;
		...
		case TOP:
			setOffsetPixels(Math.min(Math.max(mOffsetPixels + dy, 0),
					mSwipeBackViewSize));
			break;
	...
	protected void onUpEvent(int x, int y) {
		...
		case LEFT: {
			if (mIsDragging) {
				...
				mLastMotionX = x;
				animateOffsetTo(initialVelocity > 0 ? mSwipeBackViewSize : 0,
		...
		case RIGHT: {
			...
			if (mIsDragging) {
				...
				final int initialVelocity = (int) getXVelocity(mVelocityTracker);
				mLastMotionX = x;
				animateOffsetTo(initialVelocity > 0 ? 0 : -mSwipeBackViewSize,
						initialVelocity, true);
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if (action == MotionEvent.ACTION_DOWN && mSwipeBackViewVisible
				&& isCloseEnough()) {
			setOffsetPixels(0);
			stopAnimation();
			endPeek();
		...
		if (!mSwipeBackViewVisible && !mIsDragging
		...
		case MotionEvent.ACTION_DOWN: {
			...
			if (allowDrag) {
				setDrawerState(mSwipeBackViewVisible ? STATE_OPEN
						: STATE_CLOSED);
				stopAnimation();
				endPeek();
		...
		case MotionEvent.ACTION_MOVE: {
			...
			if (checkTouchSlop(dx, dy)) {
				if (mOnInterceptMoveEventListener != null
						&& (mTouchMode == TOUCH_MODE_FULLSCREEN || mSwipeBackViewVisible)
